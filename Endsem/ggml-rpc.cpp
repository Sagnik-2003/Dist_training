...
static bool send_rpc_cmd_with_retry(const std::shared_ptr<socket_t> & sock, const std::string & endpoint, 
    uint8_t cmd, const void * data, size_t size, 
    void * output, size_t output_size, int max_retries = 3) {
for (int attempt = 0; attempt < max_retries; attempt++) {
if (attempt > 0) {
GGML_LOG_INFO("Retrying connection to %s (attempt %d/%d)\n", endpoint.c_str(), attempt+1, max_retries);
// Add a small delay between retries
std::this_thread::sleep_for(std::chrono::milliseconds(100 * attempt));
}

// Try to reconnect if this isn't the first attempt
std::shared_ptr<socket_t> current_sock = (attempt == 0) ? sock : get_socket(endpoint);
if (!current_sock) {
continue; // Couldn't connect, try again
}

if (send_rpc_cmd(current_sock, cmd, data, size, output, output_size)) {
return true;
}
}
...
static enum ggml_status ggml_backend_rpc_graph_compute(ggml_backend_t backend, ggml_cgraph * cgraph) {
    ...
    if (!status) {
        GGML_LOG_ERROR("RPC connection to %s failed - backend disconnected\n", rpc_ctx->endpoint.c_str());
